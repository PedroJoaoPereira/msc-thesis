...
int openmp_scale(ImageInfo src, ImageInfo dst, int operation){
    ...
    if(src.width < 0 || src.height < 0 || dst.width < 0 || dst.height < 0){
        cerr << "[OPENMP] One of input dimensions is negative!" << endl;
        return -1;
    }
    if(src.width % 2 != 0 || src.height % 2 != 0 || dst.width % 2 != 0 || dst.height % 2 != 0){
        cerr << "[OPENMP] One of the input dimensions is not divisible by 2!" << endl;
        return -1;
    }
    if(!src.frame->data || !src.frame->linesize || !dst.frame->data || !dst.frame->linesize){
        cerr << "[OPENMP] One of the input parameters is null!" << endl;
        return -1;
    }
    ...
    if(src.width == dst.width && src.height == dst.height){
        ...
        if(openmp_scale_aux(src.width, src.height, src.pixelFormat, src.frame->data, src.frame->linesize,
