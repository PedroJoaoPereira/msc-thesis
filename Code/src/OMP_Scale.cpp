...
int omp_resampler(int srcWidth, int srcHeight, int srcPixelFormat, uint8_t* srcSlice[], int srcStride[],
...
template <class PrecisionType>
int omp_preCalculateCoefficients(int srcSize, int dstSize, int operation, int pixelSupport, PrecisionType(*coefFunc)(PrecisionType), PrecisionType** &preCalculatedCoefs) {
    ...
    for (int lin = 0; lin < preCalcCoefSize; lin++) {
        ...
        PrecisionType linNearestLeft = static_cast<PrecisionType>(floor(linOriginal)) + static_cast<PrecisionType>(0.5);
        PrecisionType linNearestRight = static_cast<PrecisionType>(ceil(linOriginal)) + static_cast<PrecisionType>(0.5);

        // Calculate distance to left nearest pixel
        PrecisionType dist = linOriginal - 1.;// static_cast<PrecisionType>(linNearest);
        ...
        for (int index = 0; index < pixelSupportDiv2; index++) {
            preCalculatedCoefs[lin][pixelSupportDiv2 - index - 1] = coefFunc(linOriginal - linNearestLeft);
            preCalculatedCoefs[lin][index + pixelSupportDiv2] = coefFunc(linNearestRight - linOriginal);
...
template <class PrecisionType>
int omp_scale_aux(AVFrame* src, AVFrame* dst, int operation) {
    ...
    if (!isOnlyResample) {
        ...
        if (omp_resampler<PrecisionType>(srcWidth, srcHeight, srcFormat, src->data, src->linesize,
        ...
        omp_resize<PrecisionType>(srcWidth, srcHeight, resampleFrame->data[0],
        ...
        for (int colorChannel = 1; colorChannel < 3; colorChannel++) {
            omp_resize<PrecisionType>(static_cast<int>(srcWidth * tempWidthRatio), static_cast<int>(srcHeight * tempHeightRatio), resampleFrame->data[colorChannel],
...
int omp_scale(AVFrame* src, AVFrame* dst, int operation) {
    ...
    if (src == nullptr || dst == nullptr) {
        cerr << "[OpenMP] One or both input frames are null!" << endl;
        return -1;
    }
    ...
    retVal = omp_scale_aux<double>(src, dst, operation);
    if (retVal < 0) {
        string error = "[OpenMP] Operation could not be done (";
